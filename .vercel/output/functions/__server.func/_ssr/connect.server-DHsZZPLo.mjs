...
//#region node_modules/.nitro/vite/services/ssr/assets/connect.server-DHsZZPLo.js
var CLIENT_ID_RE = /^[a-z0-9_]+$/;
var MCC_ID = "5321450531";
var MCC_DASHED = "532-145-0531";
var MCC_NAME = "Fago Agency";
var API_VERSIONS = [
	"v22",
	"v21",
	"v20",
...
var SECRET_DIR = "/workspace/.secrets";
var SECRET_FILE = join(SECRET_DIR, "adsops-google-ads.json");
var PUBLIC_DIR = "/workspace/public/adsops";
var KNOWN = {
	"6810292395": {
		client_id: "tkqc_6810292395",
		display_name: "GrowVi"
	},
	"2204136068": {
...
function redact(text) {
	...
	if (/ya29\.|1\/\/|client_secret|developer.token|refresh_token|Bearer |ENOENT|spawn python/i.test(text)) {
		if (/ENOENT|spawn python/i.test(text)) return "Không gọi được bộ Python cũ. Đã chuyển sang gọi Google Ads trực tiếp — bấm lại Lưu trên máy rồi thử gọi.";
...
function nowIso() {
	...
	return `${new Date(d.getTime() + off * 6e4).toISOString().slice(0, 19)}${sign}${hh}:${mm}`;
}
function parseYamlPieces(text) {
	const out = {};
...
function saveSecrets(next) {
	...
	try {
		chmodSync(SECRET_FILE, 384);
	} catch {}
}
function mergeSecrets(prev, yamlText, pieces) {
...
function item(id, label, present, required, source = "") {
	return {
		id,
		label_vi: label,
		present,
...
async function accessToken(secrets) {
	...
	if (json.error === "invalid_grant") return {
		status: "TOKEN_EXPIRED",
		error: "Refresh token hết hạn hoặc bị thu hồi. Lấy token mới (OAuth consent In production thì token mới bền). Không dán token vào chat."
	};
	if (json.error === "invalid_client") return {
		status: "PERMISSION_DENIED",
		error: "OAuth Client ID / Secret không đúng với Google Cloud project."
	};
	return {
		status: "PERMISSION_DENIED",
		error: "Không lấy được quyền Google. Kiểm tra 3 ô OAuth trên tab Kết nối."
	};
}
function adsHeaders(token, loginId, developerToken) {
	const headers = {
...
async function adsFetch(version, path, token, loginId, init, developerToken) {
	const res = await fetch(`https://googleads.googleapis.com/${version}${path}`, {
		...init,
		headers: {
			...adsHeaders(token, loginId, developerToken),
			...init?.headers || {}
		}
	});
...
async function pickVersion(token, loginId, developerToken) {
	let lastStatus = 0;
	let lastErr = "";
	for (const version of API_VERSIONS) {
		...
		const names = Array.isArray(hit.json.resourceNames) ? hit.json.resourceNames : [];
		if (hit.ok) return {
			version,
			resourceNames: names
		};
		...
		if (hit.status === 401 || hit.status === 403) {
			if (/PERMISSION_DENIED|not.*approved|access.*level/i.test(err)) return {
				version,
				status: "PERMISSION_DENIED",
				error: "Google Cloud project chưa được cấp quyền Google Ads API (Basic/Standard). Bật API trên Cloud Console, đăng nhập đúng Google có quyền MCC."
			};
		}
		if (hit.ok === false && hit.status >= 500) continue;
		if (names.length || hit.status === 200) return {
			version,
			resourceNames: names
		};
	}
	if (lastStatus === 401) return {
		version: "v22",
		status: "PERMISSION_DENIED",
		error: "Google từ chối quyền. Kiểm tra OAuth và Google có quyền MCC."
	};
	return {
		version: "v22",
...
async function searchRows(version, customerId, token, loginId, query, developerToken) {
	const rows = [];
	let pageToken = "";
	for (let i = 0; i < 20; i += 1) {
		const hit = await adsFetch(version, `/customers/${customerId}/googleAds:search`, token, loginId, {
			method: "POST",
		...
		if (!hit.ok) break;
		const results = Array.isArray(hit.json.results) ? hit.json.results : [];
		rows.push(...results);
...
function slugFor(customerId, apiName = "") {
...
function publicConnect(clientId, opts) {
	const cid = opts.account?.customer_id || (clientId === "fago_group" ? "2204136068" : digits(clientId.replace(/^tkqc_/, "")) || "");
	const display = opts.account?.display_name || KNOWN[cid]?.display_name || (clientId === "fago_group" ? KNOWN["2204136068"].display_name : "GrowVi");
	return {
		client_id: clientId,
...
function persistRoster(accounts, accessibleCount, complete) {
	const clients = accounts.filter((a) => !a.is_manager).map((a) => ({
	...
	const mcc = {
		...
		pulled_at: nowIso(),
		note_vi: complete ? `Đã kéo ${clients.length} tài khoản quảng cáo từ MCC ${MCC_DASHED}.` : "Chưa kéo đủ danh sách MCC.",
		accounts
	};
	const patchClients = (file, extra) => {
...
async function installAndProbe(input) {
	...
	try {
		...
		if ("error" in picked) return {
			...
			error_vi: picked.error
		};
		const accessibleIds = picked.resourceNames.map((n) => digits(n.split("/").pop())).filter(Boolean);
		const rows = await searchRows(picked.version, loginId, tokenHit.token, loginId, [
			"SELECT customer_client.id, customer_client.descriptive_name, customer_client.manager,",
			"customer_client.status, customer_client.currency_code, customer_client.time_zone, customer_client.level",
			"FROM customer_client"
		].join(" "), secrets.developer_token);
		const accounts = [];
		...
		for (const row of rows) {
			...
			accounts.push({
				...
				is_manager: manager,
				currency: cc.currencyCode ? String(cc.currencyCode) : void 0,
				time_zone: cc.timeZone ? String(cc.timeZone) : void 0,
				in_system: !manager
			});
		}
		...
		const current = accounts.find((a) => a.client_id === clientId) || accounts.find((a) => a.customer_id === (clientId === "fago_group" ? "2204136068" : digits(clientId.replace(/^tkqc_/, ""))));
		return {
			install: {
				ok: true,
				yaml_file_present: true
			...
			connect: publicConnect(clientId, {
				status: "CONNECTED",
				title: "Engine gọi được Google Ads API",
				detail: clients.length ? `Đã đọc MCC ${MCC_DASHED} — ${clients.length} tài khoản quảng cáo. Không apply. Chưa kéo search term / FINAL.` : `Gọi API được nhưng chưa liệt kê được tài khoản con của MCC ${MCC_DASHED}. Không đoán tên.`,
				next: clients.length ? "Chọn tài khoản trên đầu trang. Kéo kho Phân tích từng khách — không đoán số." : "Thử gọi lại. Nếu vẫn trống: Google đăng nhập chưa thấy MCC.",
				secrets,
				account: current,
		...
		return {
			install: null,
			connect: null,
			error_vi: redact(err instanceof Error ? err.message : "Lỗi gọi probe.")
		};
	}
}
async function pullKpis(clientId) {
	const id = clientId.trim();
	if (!CLIENT_ID_RE.test(id)) return {
		ok: false,
		error_vi: "Khách không hợp lệ."
	};
	return {
...
//#endregion
export { installAndProbe, pullKpis };

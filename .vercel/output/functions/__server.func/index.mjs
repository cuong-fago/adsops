...
//#endregion
//#region server/middleware/adsops-json-guard.ts
/**
...
*/
function adsopsJsonGuard(event, next) {
	if ((event.req.method ?? "GET").toUpperCase() !== "GET") return next();
	const path = event.url.pathname;
	if (path.startsWith("/adsops/") && path.endsWith(".json")) return new Response("Not found", {
		status: 404,
		headers: {
...
var globalMiddleware = [toEventHandler(adsopsJsonGuard), toEventHandler(grokPwaMiddleware)].filter(Boolean);
//#endregion
